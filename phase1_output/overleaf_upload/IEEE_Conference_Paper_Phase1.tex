...
The mechanical reinforcement mechanisms in polymer-clay nanocomposites depend critically on clay dispersion state, with exfoliated morphologies yielding superior reinforcement \cite{10, 25}. Epoxy-clay systems have received extensive attention \cite{92, 59, 15, 24, 55}, with studies exploring effects under various conditions including water absorption \cite{26, 39, 65} and elevated temperatures \cite{70}. Polyamide-clay nanocomposites represent another well-studied system \cite{80, 64, 88, 107, 117, 129, 133}.

\subsection{Surface Modification Effects}

Surface modification of clay nanoparticles enhances interfacial adhesion through organomodification with quaternary ammonium compounds \cite{138, 134}. Rauschendorfer et al. \cite{1} demonstrated substantial modulus improvements in PMA systems through controlled surface functionalization. The effectiveness varies with polymer characteristics \cite{69, 90, 107}.

\subsection{Modeling Approaches}

Various modeling approaches predict nanocomposite properties, including micromechanical models considering percolation and interphase effects \cite{8, 9, 12, 13, 11, 17}, multiscale approaches \cite{22}, and machine learning methods \cite{79, 116}.

\section{Methodology}

\subsection{Dataset Compilation and Clustering Basis}

The foundation of this analysis rests upon a curated dataset comprising 927 nanocomposite samples extracted from 148 peer-reviewed publications \cite{1}-\cite{148}. These samples were categorized based on surface modification status, yielding 789 samples (85.1\%) with modified nanofillers and 138 samples (14.9\%) with unmodified nanofillers. Prior to analysis, the dataset underwent preprocessing steps including removal of entries with incomplete mechanical property data, standardization of numeric formats, and handling of missing values through listwise deletion.
...
The vertex sets are defined as follows. \textbf{Polymer Matrix Nodes ($U$):} Each unique polymer matrix material within a cluster is represented as a node, encoding the material identity and average elastic modulus. \textbf{Composite Sample Nodes ($V$):} Individual nanocomposite samples constitute the second node set, with each node encoding the specific combination of matrix, nanofiller type, loading fraction, and modification status.

The edge set $E$ connects each composite sample to its corresponding base polymer matrix. For a composite sample $v_j$ fabricated from polymer matrix $u_i$, an edge $e_{ij} \in E$ is established with weight:
\begin{equation}
w(e_{ij}) = \left| \frac{E_c - E_m}{E_m} \times 100 \right|
\end{equation}
where $E_c$ denotes the elastic modulus of the nanocomposite and $E_m$ represents the modulus of the neat polymer matrix. This weighting scheme quantifies the absolute percentage change in elastic modulus. Note that absolute values are used to capture the magnitude of property change; however, the vast majority of samples (>95\%) exhibit positive reinforcement effects, with only a small fraction showing modulus reduction due to factors such as poor dispersion or matrix degradation.

\subsection{Centrality Evaluation Metrics}

Three complementary centrality metrics were computed to characterize polymer matrix nodes. \textbf{Degree Centrality} measures the fraction of nodes to which a given node is connected:
\begin{equation}
C_D(v) = \frac{\deg(v)}{n-1}
\end{equation}

\textbf{Weighted Degree Centrality} extends the basic degree measure by incorporating edge weights, serving as the primary indicator for identifying knowledge hubs:
\begin{equation}
...
In the context of our bipartite structure where each composite connects to exactly one polymer, degree centrality reflects the number of reported formulations per polymer (research intensity), weighted degree represents cumulative property improvements (total reinforcement magnitude), and betweenness captures within-cluster path connectivity.

\section{Results and Discussion}

\subsection{Cluster Characteristics and Graph Statistics}

The four modulus-based clusters exhibit distinct characteristics in terms of sample distribution, polymer diversity, and network structure. Table~\ref{tab:graph_stats} summarizes the graph statistics for each cluster. A clear inverse relationship emerges between matrix stiffness and improvement magnitude. Soft elastomeric systems (C1) achieve an average improvement of 351.3\%, while rigid polymers (C4) show more modest improvements of 33.9\% on average. This pattern aligns with theoretical expectations: softer matrices have greater capacity for relative stiffness enhancement through the incorporation of rigid clay platelets \cite{11, 14}.
...
Fig.~\ref{fig:cluster_graphs} presents the bipartite network visualizations for all four clusters, illustrating the polymer-composite relationships and modification patterns within each stiffness regime. The network density increases progressively from C1 to C4, reflecting the greater research emphasis on rigid polymer systems.
...
The centrality analysis identified distinct knowledge hubs within each cluster, as presented in Table~\ref{tab:centrality}. In Cluster C1, PA6 (polyamide 6) emerges as the primary knowledge hub with the highest weighted degree centrality (3822.86), indicating exceptional property improvements in soft matrix formulations \cite{64, 80}. This finding requires clarification: while bulk PA6 typically exhibits modulus values of 2-3 GPa, certain PA6 samples in the dataset represent plasticized or moisture-conditioned grades with significantly reduced modulus values falling within the $E_m \leq 0.1$ GPa range. These low-modulus PA6 variants demonstrate remarkable reinforcement potential when combined with clay nanofillers.
...
Cluster C2 identifies PMA as the dominant hub with weighted degree of 12318.18 \cite{1}, reflecting substantial property improvements through surface functionalization. For C3 and C4, epoxy emerges as the primary knowledge hub \cite{92, 47, 49} with the highest degree centrality (0.237 and 0.268), indicating extensive experimental investigation.

\subsection{Surface Modification Effectiveness}

A key finding concerns the stiffness-dependent effectiveness of surface modification strategies. Table~\ref{tab:modification} presents the comparison between modified and unmodified systems, including sample counts ($n_{mod}$ and $n_{unmod}$) to provide context for interpretation. Surface modification provides the most significant benefit in Cluster C2 (semi-soft thermoplastics), where modified systems achieve 222.6\% higher improvement than unmodified counterparts; however, this comparison is based on only 3 unmodified samples and should be interpreted cautiously. The C4 cluster provides the most statistically robust comparison with 643 modified and 114 unmodified samples.
...
\toprule
Cluster & $n_{mod}$ & $n_{unmod}$ & Mod. Avg. (\%) & Unmod. Avg. (\%) & Diff. (\%) \\
\midrule
...
In C1 and C4, unmodified systems show higher average improvements, possibly reflecting inherent matrix-clay compatibility \cite{65, 19}. The inverse relationship between matrix stiffness and improvement magnitude follows composite mechanics: softer matrices show greater relative enhancement due to larger modulus contrast with rigid clay platelets \cite{11, 14, 22}.

\section{Limitations}

Several limitations should be acknowledged. The bipartite graph structure results in disjoint star components where centrality metrics primarily reflect sample counts and cumulative improvements rather than emergent network structure. Confounding factors (clay loading, morphology, processing conditions) are not controlled; meta-regression would strengthen causal interpretability but falls outside this exploratory scope. Sample sizes for unmodified composites are limited in certain clusters (C2: n=3), constraining statistical power. The edge weight formulation uses absolute values; future work should separately report positive and negative modulus changes.

\section{Conclusion}

This study demonstrated the utility of graph-theoretic analysis for systematically evaluating polymer-clay nanocomposite research data compiled from 148 publications. The modulus-based clustering approach provides a physically meaningful framework for comparing materials within similar stiffness regimes, while bipartite graph construction captures the relationships between polymer matrices and their composite formulations.

Key findings include: (1) An inverse relationship between matrix stiffness and improvement magnitude, with soft systems (C1-C2) achieving 288-351\% average improvements compared to 33-46\% for rigid polymers (C3-C4); (2) Identification of knowledge hubs including PA6 (C1), PMA (C2), and Epoxy (C3-C4) as polymer systems with substantial cumulative property improvements; (3) Stiffness-dependent surface modification effectiveness, with semi-soft thermoplastics (C2) showing the greatest benefit from modification (+222.6\%), though this observation requires validation with larger unmodified sample sizes.

The methodology presented here provides a systematic framework for organizing heterogeneous literature data and identifying research patterns in the nanocomposites field. Future work will extend this approach to include confounding variable analysis, normalized effect size measures, and predictive modeling capabilities.
