...
## 7.x.11b Centrality glossary cross-reference

Throughout §7.x.12–§7.x.18 the terms *node*, *edge*, *edge weight*, *cluster*, *property layer*, *knowledge hub*, *weighted degree*, *betweenness*, *eigenvector centrality*, *clustering coefficient*, *cosine similarity*, and *motif* are used in the precise sense fixed by the supervisor-provided reference document "Graph and Centrality Concepts in Berfin's Thesis". The full glossary, including the *how to interpret without overclaiming* caveats, is reproduced verbatim in `thesis/CHAPTER_03_GRAPH_GLOSSARY.md`. The two interpretive rules that bind every subsequent claim in this chapter are: (i) a high weighted degree does *not* automatically mean the best material — it must be cross-checked against data count, outliers, and ductility loss; and (ii) every graph-driven hypothesis is presented as a *data-consistent and testable candidate that has not yet been experimentally verified*, never as a proven mechanism.
...
The integrated three-property analysis was conducted on the subset of 429 samples for which elastic modulus, strength, and strain-to-failure improvements were simultaneously available, corresponding to 45.5 % of the full dataset of 942 rows. After the PMA modulus and strain unit correction described in §7.x.12, this subset partitions into Phase-2 clusters as C1 = 13, C2 = 13, C3 = 36, C4 = 367. The C1 and C2 clusters together carry only 26 samples, so they are reported in this thesis as *exploratory, hypothesis-generating clusters* rather than as fully-powered statistical groups; the quantitative claims rest principally on C3 and especially on C4, which contains 367 of the 429 analysable samples.

Combining the corrected cluster populations, the normalized weighted-degree analysis, and the outlier-robust statistics, the design recommendation splits between two complementary objectives.

**Best balanced mechanical system (all three properties simultaneously improved).** The C1 modified low-modulus regime is the only cluster in which the integrated three-property analysis shows simultaneously positive $\Delta E$, $\Delta\sigma$, and $\Delta\varepsilon$ for the available samples (cluster-mean $\Delta E = +708\%$, $\Delta\sigma = +1134\%$, $\Delta\varepsilon = +77\%$). Modified-MMT-reinforced low-modulus matrices in the soft-elastomeric or low-modulus-epoxy / PMA-grafted-MMT family are therefore the recommended starting point when the design objective is balanced stiffness, strength, *and* ductility improvement. Because C1 contains only thirteen analysable samples, this cluster should be presented as a high-confidence *direction* — a hypothesis-generating regime — rather than as a fully-quantified design rule.

**Best stiffening / strength system (modulus and strength up, with an acceptable ductility penalty).** After the data correction the C2 semi-soft thermoplastic cluster, no longer hosting the PMA outlier, shows only modest mean improvements ($\Delta E \approx +51\%$, $\Delta\sigma \approx +36\%$, $\Delta\varepsilon \approx -25\%$) and is therefore not the design target for high stiffness. The corrected analysis instead identifies the *high-loading edge of the C1 PMA series* — specifically PMA-grafted MMT at filler loadings around 16 wt%, in line with the toughness optimum reported by Rauschendorfer (2020) — as the recommended high-stiffness, acceptable-ductility design point. In the words of the supervisor's outlier review: *"the most dramatic reinforcement route is observed in very soft PMA-like matrices functionalized with PMA-grafted MMT nanosheets. These systems can transform a low-modulus, highly ductile polymer into a substantially stronger and stiffer nanocomposite. However, maximum filler loading leads to severe ductility loss; therefore, an intermediate PMA-grafted MMT content around 16 wt% is predicted as the most balanced mechanical design point."* For engineering-grade rigid systems, the C4 cluster identifies PA6-, Epoxy-, and DGEBA-based Modified-MMT formulations as reliable stiffness-dominated knowledge hubs, but these systems are *not* recommended when ductility retention is a primary objective; the rigid regime is, on the corrected data, stiffness-dominated and ductility-penalised.
